...
class TestNetSheetRepository:
    def test_get_or_create_creates_new_sheet(self, subject: NetSheetRepository):
        result = subject.get_or_create("prop-1", "brokerage-1")

        assert result.id is not None
        assert result.property_id == "prop-1"
        assert result.brokerage_id == "brokerage-1"

    def test_get_or_create_returns_existing_sheet(self, subject: NetSheetRepository):
        first = subject.get_or_create("prop-1", "brokerage-1")

        second = subject.get_or_create("prop-1", "brokerage-1")

        assert second.id == first.id

    def test_get_by_property_id_returns_none_when_not_found(self, subject: NetSheetRepository):
        result = subject.get_by_property_id("nonexistent", "brokerage-1")

        assert result is None

    def test_get_by_property_id_returns_sheet_for_correct_brokerage(self, subject: NetSheetRepository):
        subject.get_or_create("prop-1", "brokerage-1")

        result = subject.get_by_property_id("prop-1", "brokerage-1")

        assert result is not None
        assert result.property_id == "prop-1"

    def test_get_by_property_id_returns_none_for_wrong_brokerage(self, subject: NetSheetRepository):
        subject.get_or_create("prop-1", "brokerage-1")

        result = subject.get_by_property_id("prop-1", "brokerage-2")

        assert result is None

    def test_add_scenario_persists_scenario(self, subject: NetSheetRepository):
        sheet = subject.get_or_create("prop-1", "brokerage-1")
    ...
    def test_get_scenarios_isolated_to_sheet(self, subject: NetSheetRepository):
        sheet1 = subject.get_or_create("prop-1", "brokerage-1")
        sheet2 = subject.get_or_create("prop-2", "brokerage-1")
