...
class TestConsumerRoutes:
    def test_get_property_returns_agent_name_not_email(
        self, subject, mock_consumer_session, mock_property_repository, mock_user_repository
    ):
        mock_consumer_session.property_id = "prop-1"
        mock_consumer_session.user_id = "user-1"
        mock_consumer_session.role = "listing_agent"
    ...
    def test_get_property_returns_none_agent_name_when_no_user(
        ...
        mock_consumer_session.property_id = "prop-1"
        mock_consumer_session.user_id = None
        mock_consumer_session.role = "listing_agent"
    ...
    def test_get_net_sheet_returns_200_for_listing_agent(self, subject, mock_consumer_session, mock_net_sheet_service):
        mock_consumer_session.id = "rep-1"
        mock_consumer_session.brokerage_id = "brk-1"
        mock_consumer_session.role = "listing_agent"
    ...
    def test_get_net_sheet_returns_403_for_buyers_agent(self, subject, mock_consumer_session):
        mock_consumer_session.role = "buyers_agent"
    ...
    def test_get_documents_returns_only_consumer_visible(
        self, subject, mock_consumer_session, mock_document_repository
    ):
        mock_consumer_session.property_id = "prop-1"
    ...
    def mock_consumer_session(self):
        return MagicMock(spec=Representation)
