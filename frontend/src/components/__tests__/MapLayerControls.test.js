...
describe('MapLayerControls', () => {
  ...
  it('sets aria-pressed true on active segments', () => {
    ...
    render(
      <MapLayerControls groups={GROUPS} isActive={isActive} onToggle={jest.fn()} />
    );
    expect(screen.getByRole('button', { name: 'Violent Crime' })).toHaveAttribute('aria-pressed', 'true');
    expect(screen.getByRole('button', { name: 'Property Crime' })).toHaveAttribute('aria-pressed', 'false');
  });
